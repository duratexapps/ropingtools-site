...
| `rotationSize` (nullable) | number | **NEW, added 2026-07-23.** Producer's target teams-per-rotation for a large field (real confirmed scenario: single classes with 200-500+ teams, sometimes run across multiple arenas). Purely a display/pacing label computed at read time over the SAME static draw order (`matching-engine.jsw`'s output) — not stored per-team, not dynamic, and does not track catches, advancement, buy-backs, or results. That stays the producer's own manual, in-arena process, same established boundary as `incentiveCapNumber` above. See `producer-draw-sheet-review.js`'s `assignRotations()` |
